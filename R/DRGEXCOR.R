#' z-score calibratin
#'
#' @param NumberVector NumberVector input
#'
#' @return z-score NumberVector
#' @export
#'
#' @examples cal_z_score(NumberVector)
cal_z_score <- function(NumberVector){
  (NumberVector - mean(NumberVector)) / sd(NumberVector)
}
...
Func_CorrPlotInputDF <- function(DNARepairGene="AAAS",XAxisGene="TREM1", MyExpData=CSCEExpData) {
  ...
  # MyExpData_RowHugoSymb <- mutate_all(MyExpData_RowHugoSymb, function(x) as.numeric(as.character(x)))
...
#' @param XAxisGene TREM1
#' @param CancerType CSCE, AML, LHC, and LSCC
#'
#' @return a plot
#' @export
#'
...
Func_CorPlot_v2 <- function(ExpData=CSCE_ProcExp[[DNARepairGene]],  YAxisGene=DNARepairGene, XAxisGene="TREM1", CancerType="CSCE" ) {
  ...
  CorrTest <- cor.test(ExpData[, colnames(ExpData)==XAxisGene], ExpData[, colnames(ExpData)==YAxisGene], methods=spearman)
  ...
  MyDotPlot <- ggplot2::ggplot(ExpData, ggplot2::aes(Gene_TREM1, DNARepairGene))+ ggplot2::geom_point( alpha=1, size=1, show.legend=T) +  # increase 'alpha' to make the color stronger. 1 is maximum.
    ...
    ggplot2::labs(x=paste0("TREM1 expression"), y=paste0(YAxisGene, " expression"),
...
#' @param YAxisGene DNARepairGene
#'
#' @return an empty plot
#' @export
#'
#' @examples Func_EmptyPlot(EmptyDF, Xaxis,Yaxis, YAxisGene=DNARepairGene)
Func_EmptyPlot <- function(EmptyDF, Xaxis,Yaxis, YAxisGene=DNARepairGene) {
  options(repr.plot.width=25, repr.plot.height=5)

  MyDotPlot <- ggplot2::ggplot(EmptyDF, ggplot2::aes(Xaxis, Yaxis)) + ggplot2::geom_point( alpha=0, size=1, show.legend=T) +  # , ggplot2::aes(color=MyColor) # increase 'alpha' to make the color stronger. 1 is maximum.
    ggplot2::xlim(c(-3, 4)) + ggplot2::ylim(c(-6.5, 4 )) + ggplot2::guides(alpha = "none", color = "none") +
    ggplot2::labs(x=paste0("TREM1 expression"), y=paste0(YAxisGene, " expression"),
         title=paste0("This DNA reapir gene, ", YAxisGene, "\ndoesn't exist in the expression data" )) +  ggplot2::theme_bw() +
    # ggtitle("<span style='font-size: 10pt;'> This DNA reapir gene doesn't exist in the expression data </font>") + theme(plot.title = element_markdown())  + geom_blank()  # + theme_bw()
    ggplot2::theme(#legend.title="MacroFraction", #element_blank(),  #legend.position="none",
      axis.text.x=ggplot2::element_text(colour="Black",size=10,angle=0,hjust=.5,vjust=.5,face="plain"),  #face="plain" or "bold"
      axis.text.y=ggplot2::element_text(colour="Black",size=10,angle=0,hjust=.5,vjust=.5,face="plain"),
      plot.title = ggplot2::element_text(size=14),
...
ShinySever <- function(input, output) {
  shiny::observe({
    # MyDNARepairGene <- DNARepairGMTDataList[[1]][1]; print(MyDNARepairGene) ## AAAS # MyDNARepairGene <- "CCNO"
    XAxisGene="TREM1";

    Xaxis<-c(-3:4); Yaxis<-rnorm(8,0,1.5); EmptyDF<-data.frame(Xaxis, Yaxis, MyColor="white"); # rownames(EmptyDF) <- letters[1:21]
    ...
    if ( req(input$DNARepairGene) %in% CSCEExpData$Hugo_Symbol ) {
          ...
          if (input$DNARepairGene %in% colnames(CSCE_ProcExp[[1]])) {
                ## Make corr dot plot.
                ...
                CSCE_CorrDotPlot_Arrange <- EmptyPlot
          }
    } else {
          CSCE_CorrDotPlot_Arrange <- EmptyPlot
    }
    ...
    if ( req(input$DNARepairGene) %in% AMLExpData$Hugo_Symbol ) {
          ...
          if (input$DNARepairGene %in% colnames(AML_ProcExp[[1]])) {
                ## Make corr dot plot.
            ...
            AML_CorrDotPlot_Arrange <- EmptyPlot
          }
    } else {
          AML_CorrDotPlot_Arrange <- EmptyPlot
    }
    ...
    if ( req(input$DNARepairGene) %in% LHCExpData$Hugo_Symbol ) {
          ...
          if (input$DNARepairGene %in% colnames(LHC_ProcExp[[1]])) {
                ## Make corr dot plot.
            ...
            LHC_CorrDotPlot_Arrange <- EmptyPlot
          }
    } else {
          LHC_CorrDotPlot_Arrange <- EmptyPlot
    }
    ...
    if ( req(input$DNARepairGene) %in% LSCCExpData$Hugo_Symbol ) {
          ...
          if (input$DNARepairGene %in% colnames(LSCC_ProcExp[[1]])) {
                ## Make corr dot plot.
            ...
            LSCC_CorrDotPlot_Arrange <- EmptyPlot
          }
    } else {
          LSCC_CorrDotPlot_Arrange <- EmptyPlot
    }
